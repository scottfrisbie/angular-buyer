			...
			.then(function(data) {
				if (ocCheckoutPaymentService.PaymentsExceedTotal(data, $scope.order.Total)) {
					ocCheckoutPaymentService.RemoveAllPayments(data, $scope.order)
						.then(function(data) {
							var payment = {
								Type: CheckoutConfig.AvailablePaymentMethods[0].Value,
					...
					var payment = {
						Type: $scope.methods[0].Value,
						DateCreated: new Date().toISOString(),
						CreditCardID: null,
						SpendingAccountID: $scope.spendingacct.ID,
						Description: null,
						Amount: $scope.order.Total,
						xp: {},
						Editing: true
					...
					$scope.payment = payment;
					return $scope.savePayment($scope.payment);
				}
			});
