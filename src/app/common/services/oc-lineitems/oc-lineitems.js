...
function LineItemFactory($rootScope, $q, $uibModal, OrderCloudSDK) {
    ...
    function _addItem(order, product){
        ...
        OrderCloudSDK.LineItems.Create(order.ID, li)
    ...
    function _customShipping(Order, LineItem) {
        ...
        modalInstance.result
            .then(function (address) {
                ...
                OrderCloudSDK.LineItems.SetShippingAddress(Order.ID, LineItem.ID, address)
                    .then(function () {
    ...
    function _listAll(orderID) {
        ...
        var dfd = $q.defer();
        var queue = [];
        OrderCloudSDK.LineItems.List(orderID, null, 1, 100)
            .then(function (data) {
                li = data;
                if (data.Meta.TotalPages > data.Meta.Page) {
                    ...
                    while (page < data.Meta.TotalPages) {
                        page += 1;
                        queue.push(OrderCloudSDK.LineItems.List(orderID, null, page, 100));
                    }
                }
                $q.all(queue)
                    .then(function (results) {
